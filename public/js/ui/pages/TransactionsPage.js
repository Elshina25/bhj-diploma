...
class TransactionsPage {
  ...
  registerEvents() {
    ...
    this.element.addEventListener('click', (e) => {
      e.preventDefault();
      const transactionParent = e.target.closest('.transaction__remove');
      if (transactionParent) {
        this.removeTransaction(transactionParent.dataset.id);
      }
    });
  }
   ...
   * */
  formatDate(date) {
    return date = new Date(date).toLocaleDateString('ru', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    ...
    // return date = new Date(date).toLocaleDateString() + ' в ' + new Date(date).toLocaleTimeString().slice(0, -3);
  }
  ...
  renderTransactions(data) {
    ...
    content.innerHTML = data.reduce((html, el) => {
      return html += this.getTransactionHTML(el);
    }, '');
  }
}